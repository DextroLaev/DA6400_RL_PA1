...
def sweep_run_sarsa_epsilon():
    ...
    sweep_id = wandb.sweep(sweep_config, project="MountainCar_SARSA")
...
def sweep_run_qlearning_softmax():
    ...
    sweep_id = wandb.sweep(sweep_config, project="MountainCar_QLEARNING")
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        # np.savetxt(f'results_seeds/q_learning_softmax_eval_rewards_{i}.txt', eval_rewards)
    ...
    for tau_start in [2,1]:
        for tau_end in [0.01,0.02]:
            ...
            # np.savetxt(f'results/q_learning_softmax_eval_rewards_{tau_start}_{tau_end}.txt', eval_rewards)
            # avg_rewards = running_avg_rewards(rewards)
            # eval_avg_rewards = running_avg_rewards(eval_rewards)
    ...
    for eps_start in [1,0.9]:
        for eps_end in [0.01,0.02]:
            ...
            # np.savetxt(f'results/sarsa_epsilon_eval_rewards_{eps_start}_{eps_end}.txt', eval_rewards)
            # avg_rewards = running_avg_rewards(rewards)
            # eval_avg_rewards = running_avg_rewards(eval_rewards)
